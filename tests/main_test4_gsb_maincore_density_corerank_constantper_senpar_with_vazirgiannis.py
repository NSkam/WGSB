...
def uniongraph(terms, term_freq, adjmatrix, collection_terms, union_graph_termlist_id, union_gr, id,
    ...
    for i in range(len(adjmatrix)): #auto douleuei dioti o adjacency matrix proerxetai apo ton admatrix tou pruned graph opws kai o kcore ara uparxei
        ...
        if terms[i] not in collection_terms:
            ...
            collection_term_freq.append(term_freq[i] * (term_freq[i] + 1) * 0.5* h * union_penalty)
            union_gr.add_node(terms[i], id=id)
            id += 1
        elif terms[i] in collection_terms:
            index = collection_terms[terms[i]]
            collection_term_freq[index] += term_freq[i] * (term_freq[i] + 1) * 0.5* h * union_penalty
            ...
            if i > j:
                if adjmatrix[i][j]!=0:
                    if terms[j] not in collection_terms:
                        ...
                        union_graph_termlist_id.append(id)
                        collection_term_freq.append(term_freq[j] * (term_freq[j] + 1) * 0.5 * h * union_penalty)
                        union_gr.add_node(terms[j], id=id)
                        id += 1
                    # print('kcorbool = %s and h = %d '%(str(kcorebool),h ))
                    if not union_gr.has_edge(terms[i], terms[j]):
                        union_gr.add_edge(terms[i], terms[j], weight=adjmatrix[i][j] * union_penalty)
                        #print("Calculating adj[",i,"]",j,"]: ",adjmatrix[i][j] * h)
                    elif union_gr.has_edge(terms[i], terms[j]):
                        union_gr[terms[i]][terms[j]]['weight'] += adjmatrix[i][j] * union_penalty
                        #print("Calculating adj[",i,"]",j,"]: ",adjmatrix[i][j] * h)
...
def runIt(filename, ans,window_flag, window_size, sen_par_flag, par_window_size,per_window,dot_split,file_sum_mat):
    ...
    if window_flag:
        ...
        try:
            adjmat = CreateAdjMatrixFromInvIndexWithVazirgiannisWindow(temp[0],filename,window_size)
        except MemoryError:
            sizeof_err_matrix = sys.getsizeof(adjmat)
            print(sizeof_err_matrix)
            exit(-1)
...
if menu == 1:
    ...
    for name in file_list:
        ...
        try:
            ug = uniongraph(terms, term_freq, adjmatrix, collection_terms, union_graph_termlist_id, union_graph, id,
                            collection_term_freq, maincore, kcorebool=corebool, UnionPen=UnionPen)
    ...
    precision_gsb = []
    recall_gsb = []
    precision_gsb_union_pen = []
    recall_gsb_union_pen = []
    precision_gsb_per_wind = []
    recall_gsb_per_wind = []
    precision_gsb_dot_split = []
    recall_gsb_dot_split = []
    ...
    av_precision_gsb_per_wind=[]
    av_precision_gsb_dot_split = []
    av_recall_gsb_dot_split = []
    av_recall_gsb_per_wind=[]
    av_recall_gsb_union_pen=[]
    av_precision_gsb=[]
    av_recall_gsb=[]
    av_precision_constant = []
    av_recall_constant =[]
    av_precision_sen_par = []
    av_recall_sen_par =[]
    ...
    for Query in Qtest:
        ...
        graphextention_set_based_using_union_penalty = q_D_similarities(query, documentmatrix, docs)
        sorted_graphextention_set_based_using_union_penalty = sorted(graphextention_set_based_using_union_penalty,
                                                                 key=itemgetter(1), reverse=True)
        documentmatrix = doc_rep(doc_vectors, idf_vec, W_vec2)
        graphextention_set_based_using_percent_win = q_D_similarities(query, documentmatrix, docs)
        sorted_graphextention_set_based_using_percent_wind = sorted(graphextention_set_based_using_percent_win,
                                                                 key=itemgetter(1), reverse=True)
        ...
        av_recall.append(temp_var[1])
        precision = temp_var[2]
        recall = temp_var[3]

        list1 = [x[0].replace('txtfiles\\', '') for x, y in sorted_graphextention_set_based]
        temp_var = pre_rec_calculation(list1,relevant) #temp_var[0] = average precision / temp_var[1] = average recall
        av_precision_gsb.append(temp_var[0])
        av_recall_gsb.append(temp_var[1])
        precision_gsb = temp_var[2]
        recall_gsb = temp_var[3]

        list2 = [x[0].replace('txtfiles\\', '') for x, y in sorted_graphextention_set_based_using_union_penalty]
        temp_var = pre_rec_calculation(list2,relevant) #temp_var[0] = average precision / temp_var[1] = average recall
        av_precision_gsb_union_pen.append(temp_var[0])
        av_recall_gsb_union_pen.append(temp_var[1])
        precision_gsb_union_pen = temp_var[2]
        recall_gsb_union_pen = temp_var[3]

        list3 = [x[0].replace('txtfiles\\', '') for x, y in sorted_graphextention_set_based_using_percent_wind]
        temp_var = pre_rec_calculation(list3,relevant) #temp_var[0] = average precision / temp_var[1] = average recall
        av_precision_gsb_per_wind.append(temp_var[0])
        ...
        av_recall_sen_par.append(temp_var[1])
        precision_sen_par_window = temp_var[2]
        recall_sen_par_window= temp_var[3]
        ...
        plt.plot(recall_gsb_union_pen, precision_gsb_union_pen, '-g', label='Graph extention with union penalty')
        plt.plot(recall_gsb_per_wind, precision_gsb_per_wind, '-y', label='Graph extention with percentge window')
    ...
    #print('av_precision_gsb_union_pen = ',av_precision_gsb_union_pen)
    #print('av_recall_gsb_union_pen = ',av_recall_gsb_union_pen)
    #print('av_precision_gsb_per_wind = ', av_precision_gsb_per_wind)
    #print('av_recall_gsb_per_wind = ', av_recall_gsb_per_wind)
    #print('av_precision_gsb_dot_split = ', av_precision_gsb_dot_split)
    #print('av_recall_gsb_dot_split = ', av_recall_gsb_dot_split)
    #print('av_precision_constant = ', av_precision_constant)
    ...
    with open('figures/allq/avarage.txt', 'a') as fd:
        ...
        fd.write('#------Doukas with union penalty\n')
        fd.write('av_precision_gsb_union_pen = %s\n'%str(av_precision_gsb_union_pen))
        fd.write('av_recall_gsb_union_pen = %s\n' %str(av_recall_gsb_union_pen))
        fd.write('#------Doukas with percentage window\n')
        fd.write('av_precision_gsb_per_wind = %s\n' % str(av_precision_gsb_per_wind))
        fd.write('av_recall_gsb_per_wind = %s\n' % str(av_recall_gsb_per_wind))
        fd.write('#------Doukas with dot split\n')
        fd.write('av_precision_gsb_dot_split = %s\n' % str(av_precision_gsb_dot_split))
        fd.write('av_recall_gsb_dot_split = %s\n' % str(av_recall_gsb_dot_split))
        fd.write('#------Doukas with Constant Window\n')
        fd.write('av_precision_constant = %s\n' % str(av_precision_constant))
        fd.write('av_recall_constant = %s\n' % str(av_recall_constant))
        fd.write('#------Doukas with combination of sentence and paragraph Window\n')
        fd.write('av_precision_sen_par = %s\n' % str(av_precision_sen_par))
        fd.write('av_recall_sen_par = %s\n' % str(av_recall_sen_par))
...
#here we write the results to excel in order for automation with perpose of further editing
        ...
        for i in range(0,100):
            ...
            sMcr = -av_precision[i] + av_precision_gsb_dot_split[i]
            sMdens = -av_precision[i] + av_precision_gsb_per_wind[i]
            sMm = -av_precision[i] + av_precision_gsb_union_pen[i]
            sMcon = -av_precision[i] + av_precision_constant[i]
            sMsp = -av_precision[i] + av_precision_sen_par[i]
            setMdouk.append(sMd)
            setMmain.append(sMm)
            setMdens.append(sMdens)
            setMcr.append(sMcr)
            setMcon.append(sMcon)
            setMsp.append(sMsp)
        ...
        try:
            ...
            df.to_excel(writer,sheet_name="TEST",header=True)
        writer.save()
        writer.close()
